...
function ajaxTableInputSend($input){
	...
	else if ( id_field == 912 || id_field == 913 )
	{
		...
		$('#textbox_pembanding_101[data-keterangan="0"]').val(value);
	}
	var role	= $input.attr("data-keterangan");
...
$(document).ready(function(){
	...
	{
		get_data_legalitas(role);
		var brb2 = $("#bangunan_btb_Bangunan_1").find("input#textbox_bangunan_176").val();
		var current_value = $('#textbox_pembanding_102[data-keterangan="0"]').val();
		if ( (brb2 == '' || brb2 == undefined || brb2 == null) && ( current_value == '' || current_value == undefined || current_value == null ) ) {
			getBiayaLangsung('Bangunan_1');
		} else {
			if ( current_value == '' || current_value == undefined || current_value == null )
				$('#textbox_pembanding_102[data-keterangan="0"]').attr("readonly", "readonly").addClass("text-right").number( true, 0 ).val(brb2);
		}
		get_brb_standar_mappi_dbanding();
	}
	...
	var data_href = $('.panel-head.panel-'+role).attr("href");
	...
	$("#textbox_tanah_64, #textbox_bangunan_54, #textbox_bangunan_55, #textbox_bangunan_56, #textbox_bangunan_57, #textbox_bangunan_58").number( true, 0 );
	$(".btn-tambah-bangunan").hide();
	...
	if (id != "")
	{
		$.ajax({
			...
			success		: function (data) {
				...
				input_kelas = $('#textbox_bangunan_138');
			},
		});
	}
}
	...
	if (jml != "" && tipe !="")
	{
		$.ajax({
			...
			success		: function (data) {
				...
				$("#" + role).find("th#index_koefisien_lantai").val(data);
			},
		});
	}
}
...
function kalkulasiBiaya(role){
	...
	var umur_efektif = 0;
	var tahun_penilaian = parseFloat($("#" + role).find("input#textbox_bangunan_133").val()) || 0; 
	var tahun_bangun =   parseFloat($("#" + role).find("input#textbox_bangunan_134").val()) || 0; 
	var tahun_renovasi = parseFloat($("#" + role).find("input#textbox_bangunan_136").val()) || 0;
	var jumlah_lantai = parseFloat($("#" + role).find("select#textbox_bangunan_132").val()) || 0;
	
	getKoefisien(role);

	var koefisien = $("#" + role).find("#hdnKoefisien").val();
	$("#index_koefisien_lantai").html(koefisien);
	...
	$("#" + role).find("input#textbox_bangunan_2072").val(umur_ekonomis).attr("readonly", true).addClass("readonly");
	
	var total_biaya2 = 0;
	var total_biaya1 = 0;
	var plafond=dinding=pintu_jendela=lantai=pondasi=struktur=rangka_atap=penutup_atap=utilitas=0;
	...
	$(".table_biaya_langsung_" + tab_role[1] + " tr ").find("input[id=textbox_bangunan_157]").each(function(index) {
		...
		$row.find("#textbox_bangunan_164").val(material2); 
		...
		var index_lantai_mappi = 0;
		if ( parseFloat(harga2) > 0 ) {
			index_lantai_mappi = parseFloat(harga2)*parseFloat(koefisien);
		} else if ( parseFloat(harga2) == 0 && parseFloat(harga1) > 0 ) {
			index_lantai_mappi = parseFloat(harga1)*parseFloat(koefisien);
		}
		$row.find("#index_lantai_mappi").val(index_lantai_mappi);
		...
		if ( parseFloat(harga2) > 0 ) {
			if ( parseFloat(terpasang2) == 0 ) {
				hargaPenyesuaian = parseFloat(harga1)*parseFloat(koefisien);
			} else if ( parseFloat(terpasang2) == 100 ) {
				hargaPenyesuaian = parseFloat(harga2)*parseFloat(koefisien);
			} else {
				hargaPenyesuaian = ((parseFloat(harga1)*(terpasang1/100))+(parseFloat(harga2)*(terpasang2/100)))*parseFloat(koefisien);
			}
		} else if ( parseFloat(harga2) == 0 && parseFloat(harga1) > 0 ) {
			hargaPenyesuaian = parseFloat(harga1)*parseFloat(koefisien);
		}
		...
		if (index==0 || index==1){
			...
			//hargaPenyesuaian = (parseFloat(harga1)*(parseFloat(terpasang1)/100))+(parseFloat(index_lantai_mappi)*(parseFloat(terpasang2)/100));
		}
		else
		{
			//hargaPenyesuaian = (parseFloat(harga1)*(parseFloat(terpasang1)/100))+(parseFloat(index_lantai_mappi)*(parseFloat(terpasang2)/100));
		...
		$row.find('#index_lantai_mappi').addClass("text-right").number(true, 0);		
	...
	var luas_bangunan = parseFloat($("#" + role).find("input#textbox_bangunan_5").val()) || 0;
	...
	/*console.log ("--------------------");
	console.log ("total_biaya2 813", textbox_bangunan_170);
	console.log ("total_biaya2 813", luas_bangunan_lainya);
	console.log ("total_biaya2 813", luas_bangunan);

	console.log ("total_biaya2 813", total_biaya2, textbox_bangunan_171);*/
	...
	$("#" + role).find("input#textbox_bangunan_141").val(fee1).attr("value", fee1);
	...
	var textbox_bangunan_143 = parseFloat($("#" + role).find("input#textbox_bangunan_141").val()) + parseFloat(total_biaya1);
	$("#" + role).find("input#textbox_bangunan_143").val(textbox_bangunan_143);
	...
	// var selisih = grand_1 - grand_2;
	$("#" + role).find("input#textbox_bangunan_151").addClass("readonly").addClass("text-right").number( true, 0 ).css({"background-color": "red", "font-weight": "bold", "color": "white"});
	...
	$("#" + role).find("input#textbox_bangunan_152").val(penyusutan_fisik.toFixed(1)).addClass("readonly").attr("readonly","readonly") //.updateTextbox()
	
	$("#" + role).find("input#textbox_bangunan_153").addClass("text-right").attr('maxlength',3);
	var penyusutan_terlihat = parseFloat($("#" + role).find("input#textbox_bangunan_153").val()) || 0;
	if (isNaN(penyusutan_terlihat)){
		penyusutan_terlihat=0;
	}
	$("#" + role).find("input#textbox_bangunan_154").addClass("text-right").attr('maxlength',3);
	var penyusutan_fungsional = parseFloat($("#" + role).find("input#textbox_bangunan_154").val()) || 0;
		
	if (isNaN(penyusutan_fungsional)){
		penyusutan_fungsional=0; 
	}
	$("#" + role).find("input#textbox_bangunan_155").addClass("text-right").attr('maxlength',3);
	var penyusutan_ekonomis = parseFloat($("#" + role).find("input#textbox_bangunan_155").val()) || 0;
	...
	$("#" + role).find("input#textbox_bangunan_156").addClass("readonly").addClass("text-right").val(penyusutan_total) .updateTextbox();
	...
	$("#" + role).find("input#textbox_bangunan_176").attr("readonly", "readonly").addClass("readonly").addClass("text-right").number( true, 0 ).val(brb_m2) .updateTextbox();
	$('#textbox_pembanding_102[data-keterangan="0"]').attr("readonly", "readonly").addClass("text-right").number( true, 0 ).val(brb_m2).updateTextbox();
...
function update_sarana_pelengkap(doUpdate = DONT, $input){
...

	{
		...
		{
			$("#textbox_sarana_1").val($(".input_672_Bangunan_1").val()).attr("readonly", true).addClass("readonly") .updateTextbox(doUpdate);
		}
	...
	}
	{
		...
		{
			$("#textbox_sarana_6").val($(".input_673_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
	...
	}
	{
		...
		{
			$("#textbox_sarana_28").val($(".input_676_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
		...
		{
			$("#textbox_sarana_29").val($(".input_675_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
	...
	}
	{
		...
		{
			$("#textbox_sarana_34").val($(".input_683_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
	...
	}
	{
		...
		{
			$("#textbox_sarana_46").val($(".input_671_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
	...
	}
	{
		...
		{
			$("#textbox_sarana_58").val($(".input_678_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
	...
	}
	{
		...
		{
			$("#textbox_sarana_64").val($(".input_680_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
	...
	}
	{
		...
		{
			$("#textbox_sarana_82").val($(".input_682_Bangunan_1").val()).attr("readonly", true).addClass("readonly")	.updateTextbox(doUpdate);
		}
		else
		{
...
function load_tab_bangunan(role) {
	...
	getKelasBangunan(role, input_kelas);
	getBiayaLangsung(role);
}
...
function placeMarker(location) {
    if ( typeof MyLatLng != 'undefined' ) {
        ...
        // console.log ($('#latitude_pembanding_'+active_idx));
...
$(document).on("change", '#textbox_pembanding_101, #textbox_pembanding_16', function(){
    var idx_pembanding = $(this).attr('data-keterangan');
    var jenis_bangunan = $('select#textbox_pembanding_101[data-keterangan="'+idx_pembanding+'"]').val();
    var lantai 		   = $('#textbox_pembanding_16[data-keterangan="'+idx_pembanding+'"]').val();
...
function get_brb_standar_mappi(idx_pembanding, jenis_bangunan, lantai) {
	...
	$.ajax({
		...
		success : function (d) {
			var data;
			try {
				$('#textbox_pembanding_102[data-keterangan="'+idx_pembanding+'"]').attr("readonly", "readonly").addClass("text-right").number( true, 0 ).val(d);
				$('#textbox_pembanding_24[data-keterangan="'+idx_pembanding+'"]').addClass("text-right").number( true, 0 ).val(d).updateTextbox();
			} catch(e)
			{
					//test
			}
...
function get_brb_standar_mappi_dbanding() {
	var kertas_kerja = $('#id_kertas_kerja').val();
	var jenis_bangunan = []; 
	var jumlah_lantai = [];
	$('select#textbox_pembanding_101').each(function() {
		var index_pembanding = $(this).attr('data-keterangan');
		var jenis = $(this).val();
		var lantai = $('#textbox_pembanding_16[data-keterangan="'+index_pembanding+'"]').val();
		if ( parseInt(index_pembanding) > 0 ) {
			jumlah_lantai[index_pembanding] = lantai;
			jenis_bangunan[index_pembanding] = jenis;
		}
	});

	$.ajax({
		type : "POST",
		url : base_url + "AjaxPekerjaan/get_brb_standar_mappi_dbanding/"+kertas_kerja,
		data : {
			jenis_bangunan : JSON.stringify(jenis_bangunan),
			jumlah_lantai : JSON.stringify(jumlah_lantai)
		},
		success : function (d) {
			var data;
			try {
				...
				$.each(result, function(index, value) {
					if ( parseInt(index) > 0 )
						$('#textbox_pembanding_102[data-keterangan="'+index+'"]').attr("readonly", "readonly").addClass("text-right").number( true, 0 ).val(value);
				});
			} catch(e)
			{
					//test
