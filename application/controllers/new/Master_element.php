 ...
 */
class Master_element extends CI_Controller {
    var $page_title = 'Master Element';
    var $column_datatable = array('A.id', 'A.element', 'A.bangunan', 'A.lov', 'A.index');
    ...
    function __construct(){
        ...
        $this->load->model($this->folder.'/'.'Master_element_model', 'app_model');
    ...
    public function index()
    {
        ...
        $script = '
            ...
            $(function () {
                var oTable = $("#datatable").DataTable({
                    ...
                    // "buttons": [
                    //     {
                    //         text: "<i class=\'fa fa-pencil-circle\'></i> Edit", 
        ...
        $this->template2->write_view('content', $this->folder.'/master_element/datatable', $data, true);
    ...
    function add($id_sub_perusahaan = 0) {
        $this->load->model('global_model');
        $module = $this->module;
        if ( !in_array($id_sub_perusahaan, $this->list_sub_array) ) {
        ...
        if($_POST)
        {
            ...
            if ( $simpan ) {
                $data_insert = array(
                    'id_sub_perusahaan' => $id_sub_perusahaan,
                    'master_deviasi' => strip_tags($this->input->post('master_deviasi')),
                    'is_active' => 0,
                    'posisi_data' => VERIFIKATOR_ID
                );
        ...
        $this->template2->write_view('content', $this->folder.'/master_element/form', $data, true);
        $this->template2->render();
    }
    function edit($id = 0) {
        $this->load->model('global_model');
