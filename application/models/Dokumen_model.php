...
{
    var $table      = 'view_list_dokumen';
    var $table_klien = 'mst_klien';
    var $table_lokasi = 'mst_lokasi';
    var $table_jenis_objek = 'mst_jenis_objek';
    var $table_data_legalitas = 'r_data_legalitas';
    var $pk_name    = 'id';
    var $module     = '';
    ...
    public function show_datatable($column_datatable, $iDisplayStart,$iDisplayLength,$order,$sSearch,$con){
        ...
        */
        ...
        foreach($rResult->result_array() as $aRow)
        {
            ...
            $row[] = $aRow['jenis_sertifikat'];
            $row[] = !$aRow['file_sertifikat'] ? '-' : '<a href="'.base_url().'asset/file/'.$aRow['file_sertifikat'].'">Download</a>';
            $row[] = $aRow['nama_dokumen_master'];
            $row[] = !$aRow['file_dokumen_gabung'] ? '-' : '<a href="'.base_url().'asset/file/'.$aRow['file_dokumen_gabung'].'">Download</a>';
    ...
    function get_by_id($id = '') {
        $this->db->select("*")->from($this->table)->where($this->pk_name, $id);
        $query = $this->db->get();
        if ( is_object($query) ) {
